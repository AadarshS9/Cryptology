...
print(f"Enter the elements of the {n}x{n} key matrix row-wise (space-separated integers between 0-25):")
key_matrix = []
for i in range(n):
    row = list(map(int, input().split()))
    if len(row) != n:
        print(f"Error: Please enter exactly {n} numbers for each row.")
        exit()
...
#20 9 17
#9 4 17
#ATTACK IS TONIGHT....FNWAGWJGJKDNRRQ/-
